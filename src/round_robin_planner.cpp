...
void RoundRobinPlanner::execute_processes() {
    ...
    while (!q_.empty()) {
        ...
        std::unique_lock quantum_lock{quantum_mutex_};
        time_point<steady_clock> prev = steady_clock::now();
        if (p_duration_cv.wait_for(quantum_lock,p.duration_) != std::cv_status::timeout) { // Executing process
...
void RoundRobinPlanner::quantum_clock() {
    ...
    while (q_clock_active_) {
        std::unique_lock quantum_lock{quantum_mutex_};
        if (q_duration_cv_.wait_for(quantum_lock,quantum_) == std::cv_status::timeout) //  Notify to stop the current process executing
